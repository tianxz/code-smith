 ...
 */
public class ${classInfo.name}UpdateDepict extends HashMap<String, UpdateDepict> {

    public ${classInfo.name}UpdateDepict() {
<#list classInfo.fields as field>
        this.put(${classInfo.name}Meta.${field.sqlName}_FIELD_NAME, new UpdateDepict("${field.name}", "${field.sqlName}"));
</#list>
    }
